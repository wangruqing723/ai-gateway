...
该字段同时作用于模型列表查询（`GET /api/providers/models`）与 Provider 健康检测——它们探测上游 `/v1/models`，同样受 UA 准入影响。不配的话，按客户端类型准入的上游会出现「转发正常、但查不到模型列表、且健康检测显示不健康」这种割裂状态。这两条路径由网关自己发起，没有客户端 UA 可转发，所以只有「配了就用、没配就用 Go 默认值」两种情况。
