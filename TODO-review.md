...
- ✅ **第二批（超时/流式语义对齐 Node）**：#4 #5 #6 #7 #8 #16。
...
### ✅ 3. singleflight leader 把自己的 ctx 错误广播给所有等待者 [已修复]
...
### ✅ 5. `bufio.Scanner` 4MB 单行上限静默截断流 [已修复]
...
### ✅ 6. 非流式超时返回 502/proxy_error，而非 504/timeout_error [已修复]
...
### ✅ 16. 非流式路径客户端断开和超时未区分处理 [已修复]
- **位置**：`cmd/gateway/main.go:244-246`
- **问题**：`proxy.Forward` 返回错误时一律记录为"转发结束（异常）"，未区分 `context.Canceled`（客户端主动断开）、`context.DeadlineExceeded`（真实超时）和其他转发错误。
- **后果**：客户端主动停止请求（如 Claude Code 点击停止按钮）被误报为转发异常；真实的超时错误也没有单独标注超时时长。
- **修法**：在 main handler 返回前用 `errors.Is` 区分三种情况：`context.Canceled` → "客户端断开连接"；`context.DeadlineExceeded` → "转发超时（X秒）"；其他 → "转发结束（异常）"。

---

## 🟠 行为偏离 Node 原版

### ✅ 9. 缺 `media_type` 时图片哈希与 Node 不一致 [已修复]
...
### ✅ 10. `blocksHaveImage` 递归深度与 Node 不一致 [已修复]
...
### ✅ 11. content 块 `text` 为 JSON 数字时类型断言隐患 [已修复]
...
### ✅ 12. `nextReqID` 读改写竞争产生重复请求 ID [已修复]
...
### ✅ 13. passthrough 判定逻辑三处重复且已分歧 [已修复]
