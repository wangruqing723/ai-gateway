 ...
 * @returns {Promise}
 */
export function enqueue(providerName, maxConcurrent, task, maxPerSecond = 0) {
 ...
 * 获取队列状态（用于日志）
 * @param {string} providerName
 * @returns {{ running: number, queued: number, maxConcurrent: number, maxPerSecond: number }}
 */
export function getQueueStatus(providerName) {
  const state = providerQueues.get(providerName);
  if (!state) return { running: 0, queued: 0, maxConcurrent: 0, maxPerSecond: 0 };
