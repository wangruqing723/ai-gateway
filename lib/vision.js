...
// Vision 请求超时（毫秒）
const VISION_REQUEST_TIMEOUT = 120_000; // 120 秒
...
async function callVision(imageBlock, vision) {
  ...
  const recognitionPromise = enqueue(providerName, maxConcurrent, () => {
    return new Promise((resolve, reject) => {
      ...
      // 请求超时处理
      req.on('timeout', () => {
        req.destroy(new Error(`视觉 API 请求超时 (${VISION_REQUEST_TIMEOUT / 1000}秒)`));
      });
  ...
  // 安全兜底：超时后强制清除 Map 条目，防止请求挂起导致后续请求永久阻塞
  const safetyTimeout = setTimeout(() => {
    if (pendingRecognitions.get(hash) === recognitionPromise) {
      pendingRecognitions.delete(hash);
    }
  }, VISION_REQUEST_TIMEOUT + 5000);
  safetyTimeout.unref();
  recognitionPromise.finally(() => clearTimeout(safetyTimeout));
